...
#### AWS Cognito Setup
If not already done in the previous sections, you may setup cognito for authentication for MCP Gateway.
...
Once you are done with working on `Identity and Priviege abuse`, you may also try to see if `Agent Goal Hijack` is possible in the current setup.
